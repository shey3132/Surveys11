...
// Yemot forbids '.' and extra '-' inside the spoken text (breaks the extension).
// Strip anything that could break the `t-...=` segment.
function sanitizeForSpeech(text) {
  return String(text)
    .replace(/[.\-=]/g, ' ')   // remove dot, dash, equals
    .replace(/\s+/g, ' ')
    .trim();
}
...
function buildMessage(text) {
  // Plays a message with no further input expected, then ends the call.
  // NOTE: verify exact command name/params against Yemot's own docs/support —
  // this assumes `id_list_message` behaves like a "say and hang up" command.
